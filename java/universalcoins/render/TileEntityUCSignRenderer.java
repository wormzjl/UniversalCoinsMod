...
public class TileEntityUCSignRenderer extends TileEntitySpecialRenderer {
		...
		public void renderTileEntityAt(TileUCSign tileEntity, double xCoord, double yCoord, double zCoord, float p_147512_8_) {
	        ...
	        for (int i = 0; i < tileEntity.signText.length; ++i)
	        {
	            ...
	            {
	                s = "> " + s + " <";
	                fontrenderer.drawString(s, -fontrenderer.getStringWidth(s) / 2, i * 10 - tileEntity.signText.length * 5, colors[colorCode]);
	            }
	            else  {
	            	if (s.length() <= 16) {
	            		fontrenderer.drawString(s, -fontrenderer.getStringWidth(s) / 2, i * 10 - tileEntity.signText.length * 5, colors[colorCode]);
            			...
            			fontrenderer.drawString(subset, -fontrenderer.getStringWidth(subset) / 2, i * 10 - tileEntity.signText.length * 5, colors[colorCode]);
	            		counter++;
